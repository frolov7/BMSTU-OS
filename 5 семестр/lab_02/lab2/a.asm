...
pm_seg	SEGMENT PARA PUBLIC 'CODE' USE32 ; указываем, что команды работают со всеми 32 битов регистров; например LOOP будет
    ...
    gdt_code32      descr <pm_seg_size-1,0,0,98h,0CFh,0> ; 32-битный 4-гигабайтный сегмент кода с базой PM_seg
    gdt_data32      descr <pm_seg_size-1,0,0,910010010b,0CFh,0> ; 32-битный 4-гигабайтный сегмент данных с базой PM_seg
    gdt_stack32     descr <stack_size-1,0,0,10010010b,0CFh,0> ; 32-битный 4-гигабайтный сегмент данных с базой stack_seg
...
;Ловушка — прерывание или исключение, при возникновении которого в стек записываются значения регистров cs: ip, указывающие на команду, следующую за командой, вызвавшей данное прерывание.
    ...
    ; таблица дескрипторов прерываний IDT для прерываний для корректного вычисления обработчика прерываний
    idt             label byte
    ...
    trap1           intr 13 dup (<0,sel_code32,0,10001111b,0>)
    trap13          intr <0,sel_code32,0,10001111b,0> ; Некоторые исключения кладут в стек код ошибки. В заглушке для 13 прерывания снимается со стека код ошибки.
    trap2           intr 18 dup (<0,sel_code32,0,10001111b,0>)
    ...
    realm_msg db 'Now on real mode.'
    to_to_pm_msg    db 'DOS switched to protected mode.'
    to_to_pm_msg_len=$-to_to_pm_msg
    timer_msg    db 'Timer ticks:   '
    timer_msg_len=$-timer_msg
    memory_msg    db 'Available memory in MB, hexadecimal number system, : '
...
pm_start:
    ...
    print_str to_pm_msg,to_pm_msg_len,360
    print_str timer_msg,timer_msg_len,520
    print_str memory_msg,memory_msg_len,5*160+40
...
end_of_memory:
    ...
    mov    ebp,0B8000h
    mov    ecx,8
    add    ebp,5*160+2*(memory_msg_len+7)+40
    mov    dh,attr2
cycle:
    mov    dl,al
    and    dl,0Fh
